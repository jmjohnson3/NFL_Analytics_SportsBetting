...
| `NFL_ODDSPORTAL_MANUAL_CSV` | Optional path to a user-maintained CSV of closing moneylines (columns: `season`, `home_team`, `away_team`, `home_closing_moneyline`, `away_closing_moneyline`, optional `kickoff_date`). When present, rows matching the requested season are merged as a last-resort fallback. |
